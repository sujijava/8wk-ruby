# Week 2, Day 3: Dependency Injection - Part 1 (YOUR SOLUTION)
# Refactor api_client_before.rb to use dependency injection
#
# Your refactored code should:
# 1. Inject HTTP client instead of hardcoding HTTParty
# 2. Provide default dependencies so existing code still works
# 3. Make all clients testable with mock HTTP clients
# 4. Allow swapping HTTP libraries easily

require "httparty"
# require "json"

# Copy the FakeHTTParty module from api_client_before.rb for testing
...
class TravelPlannerService
  ...
  def get_destination_info(city)
    ...
    weather_client = WeatherClient.new(@http_client)
    geocoding_client = GeocodingClient.new(@http_client)
  ...
  def calculate_trip_cost(amount, from_currency, to_currency)
    ...
    exchange_client = CurrencyExchangeClient.new(@http_client)
...
if __FILE__ == $0
  puts "Testing refactored API clients with dependency injection..."
