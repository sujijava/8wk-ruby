# Week 3, Day 1: Strategy Pattern
# Exercise: Build a shipping cost calculator that supports multiple carriers
#           (FedEx, UPS, USPS) with different rate algorithms
...
class ShippingCreator
  ...
  def create_shipping(package, carrier)
    cost = ShippingService.new.calculate_shipping(carrier, package)
    shipment = {
...
class ShippingService
  def initialize
    @strategies = {
      "fedex": FedExStrategy.new,
      "ups": UPSStrategy.new,
      "usps": USPSStrategy.new,
    }
  end

  def calculate_shipping(carrier, package)
    carrier = carrier.to_sym
    raise "Unsupported carrier: #{carrier}" if @strategies[carrier].nil?
    @strategies[carrier].calculate(package)
  end

  def create_cheapest_shipment(package)
    cheapest_carrier = cheapest_carrier(package)

    if cheapest_carrier
      ShippingCreator.new.create_shipping(package, cheapest_carrier)
    else
  ...
  def compare_carriers(package)
    ...
    @strategies.each do |carrier, strategy|
      begin
        cost = strategy.calculate(package)
        rates[carrier] = cost
  ...
  def cheapest_carrier(package)
    ...
    @strategies.each do |carrier_name, strategy|
      begin
        rates[carrier_name] = calculate_shipping(carrier_name, package)
      rescue => e
        puts "Error calculating #{carrier_name}: #{e.message}"
      end
    end

    rates.min_by { |carrier, rate| rate }&.first
  end
...
class ReportGenerator
  ...
  def generate_report(shipments)
    ...
    by_carrier.each do |carrier, group_shipments|
      total_cost = group_shipments.sum { |s| s[:cost] }
      avg_cost = total_cost / group_shipments.count

      puts "#{carrier.upcase}:"
      puts "  Shipments: #{group_shipments.count}"
...
binding.pry
  ...
  ShippingService.new.compare_carriers(small_package)

  # Create some shipments
  puts "\n\nCreating shipments..."
  puts "-" * 70
  shipping_creator = ShippingCreator.new
  shipping_creator.create_shipping(small_package, "fedex")
  ShippingService.new.create_cheapest_shipment(small_package)

  ReportGenerator.new.generate_report(shipping_creator.shipments)
end
